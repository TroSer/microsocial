...
class UserFriendShipManager(models.Manager):
    ...
    def add(self, user1, user2):
        ...
        if user1_id == user2_id:
            raise ValueError(_(u'Нельзя добавить самого себя в друзья.'))
...
class User(AbstractBaseUser, PermissionsMixin):
    ...
    is_active = models.BooleanField(_('active'), default=True,
        help_text=_('Designates whether the user should be treated as '
    ...
    about_me = models.TextField(_(u'о себе'), max_length=900, blank=True)
    interests = models.TextField(_(u'интересы'), max_length=900, blank=True)
...
class FriendInviteManager(models.Manager):
    ...
    def add(self, from_user, to_user):
        ...
        if self.is_pending(to_user_id, from_user_id):
            User.friedship.add(from_user_id, to_user_id)
            return 2
