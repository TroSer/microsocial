...
from person.models import User, FriendInvite
...
class UserFriendsView(TemplateView):
    template_name = 'person/friends_friends.html'
...
class UserIncomingView(TemplateView):
    template_name = 'person/friends_incoming.html'
...
class UserOutcomingView(TemplateView):
    template_name = 'person/friends_outcoming.html'
...
class FrienshipAPIView(View):
    @method_decorator(login_required)
    @method_decorator(require_POST)
    def dispatch(self, request, *args, **kwargs):
        method_name = '_ation_{}'.format(request.POST.get('action', ''))
    ...
    def _action_add_to_friends(self):
        ...
        if user:
            try:
                ...
                if r == 1:
                    messages.success(self.request, _(u'Заявка отправлена и ожидает рассмотрения.'))
                elif r == 2:
                    messages.success(self.request, _(u'Пользователь добавлен в друзья.'))
                    return 'user_friends'
    ...
    def _action_approve(self):
        ...
        if user:
            try:
                ...
                if r:
                    messages.success(self.request, _(u'Заявка подтверждена. Пользователь добавлен в друзья.'))
    ...
    def _action_reject(self):
        ...
        if user:
            ...
            messages.success(self.request, _(u'Заявка подтверждена.'))
        return 'user_incoming'
    ...
    def _action_cancel_outcoming(self):
        ...
        if user:
            ...
            messages.success(self.request, _(u'Заявка отменена.'))
        return 'user_outcoming'